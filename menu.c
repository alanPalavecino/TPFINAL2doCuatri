        ...
        case 1:
            ...
            if(aux){
               ...
               muestraUnNodoClienteAdmin(aux);
                ...
                {
                gotoxy(9, 24);
                printf("Ese id no pertenece a ningun cliente : ");
               }
...
void adminPedido(nodoCliente* listaCliente)
{
    ...
    do
    {
        titulo();
        gotoxy(10,10);
        printf("1.Ver pedidos"); ///HAY QUE BUSCARLO POR LA FECHA O POR EL ID BUSCA UN PEDIDO DE UN SOLO CLIENTE (SI ELIGE ESTA OPCION PRIMERO INGRESA EL ID DEL CLIENTE QUE QUIERE BUSCAR Y LUEGO LA FECHA O EL ID DEL PEDIDO)
        gotoxy(10,11);
        ...
        case 3:
            ...
            if(auxCliente->listaPedidos)
            {
                ...
                if(flag == 1)
                {
                    ...
                    printf("<<<<Se ha eliminado el pedido con exito>>>>");
                    muestraPedidosSistemaUnCliente(auxCliente);
                    persistirDatosCabeceraYDetalle(listaCliente, ARCHIVO_CABECERA_PEDIDO, ARCHIVO_DETALLE_PRODUCTO);
                }
                else
                {
...
void adminProducto(arbolProducto* productosArbol)
{
    ...
    do
    {
        ...
        printf("4.Eliminar");
        gotoxy(10,14);
        printf("5.Activar producto");
        gotoxy(10,15);
        printf("6.Ver todos los productos");
        gotoxy(10,16);
        printf("0.Salir");
        ...
        case 1:
            ///Alta producto en EL ARBOL y despues subirlo al archivo
        ...
        case 2:
            ...
            bajaProducto(ARCHIVO_PRODUCTO, guardaId);
            gotoxy(9,25);
            printf("<<<<ES NECESARIO REINICIAR EL SISTEMA PARA QUE SE CARGUEN LOS DATOS>>>>");
            getch();
        ...
        case 5:
            ///FUNCION QUE BORRA UN PRODUCTO EN ARBOL Y ARCHIVO
            gotoxy(9, 23);
            printf("Ingrese ID del producto: ");
            fflush(stdin);
            scanf("%d", &guardaId);

            aux = buscarArbolId(productosArbol, guardaId);
            productosArbol = borrarNodoArbolPorIdProducto(productosArbol, aux->datoProducto.idProducto);
            bajaProducto(ARCHIVO_PRODUCTO, guardaId);
            gotoxy(9,25);
            printf("<<<<ES NECESARIO REINICIAR EL SISTEMA PARA QUE SE CARGUEN LOS DATOS>>>>");
            getch();
